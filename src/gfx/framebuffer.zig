...
pub fn init() !Self {
    ...
    var tex_depth_buffer: c_uint = 0;

    const width: c_int = @intCast(window.get_width() catch 0);
    const height: c_int = @intCast(window.get_height() catch 0);
    ...
    gl.genTextures(1, &tex_depth_buffer);
    gl.bindTexture(gl.TEXTURE_2D, tex_depth_buffer);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.DEPTH_COMPONENT24, width, height, 0, gl.DEPTH_COMPONENT, gl.FLOAT, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);

    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.TEXTURE_2D, tex_depth_buffer, 0);
    ...
    return .{
        ...
        .tex_depth_buffer = tex_depth_buffer,
    };
}
...
pub fn deinit(self: *Self) void {
    ...
    gl.deleteTextures(1, &self.tex_depth_buffer);
    gl.deleteFramebuffers(1, &self.fbo);
}

pub fn bind(self: *Self) void {
    gl.bindFramebuffer(gl.FRAMEBUFFER, self.fbo);
    const attachment = [_]gl.GLenum{ gl.COLOR_ATTACHMENT0, gl.COLOR_ATTACHMENT1 };
